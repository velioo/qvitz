...
class Notifications extends CI_Controller {
	...
	public function load_notifications() {	
		...
		if($limit != NULL) {
			...
			if($first_load == 1) {
				...
				$notifications_per_load = $limit;
				$total_groups = ceil($total_notifications/$notifications_per_load);
					
				echo "<span>$total_groups</span>";
			}
			...
			if($unseen_count != 0 || $first_load == 1 || $group_number != NULL) {
				...
				$this->deliver_notifications($notifications);	
				
			}
			
		} else {
			$unseen_notifications = $this->notifications_model->get_unseen_notifications();
			
			$unseen_count = count($unseen_notifications);	
			
			if($unseen_count != 0) {
				$this->deliver_notifications($unseen_notifications);				
			}
		}
	...
	public function all_notifications() {
		if($this->session->userdata('is_logged_in')) {
			...
			$data['title'] = 'Qvitz-notifications';
	...
	public function deliver_notifications($notifications) {
	
		$elements = array();
		$link = "";
		$types = array('level', 'quiz_comment');
		...
		foreach($notifications as $notification) {
			...
			switch($notification['type']) {
				case $types[0]:
					...
					break;
				case $types[1]:
					$link = site_url("quiz/do_quiz/{$notification['source_id']}/{$notification['additional_info']}");				
					break;
			}
			...
			$element ='<a href="' . $link .'" class="disable-link-decoration"> <div class="notification" data-id="'. $notification['id'] .'" >
								<div class="user_image_div notify"><img src="' . asset_url() . "user_images/{$notification['avatar']}" . '" class="user_image" onError="this.onerror=null;this.src=\'' . $onerror_src .'\';" ></div>
